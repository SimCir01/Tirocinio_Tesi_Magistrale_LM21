...
simulation script create  &
   ...
   commands =   &
              "simulation single_run transient type=auto_select initial_static=no end_time=7.0 number_of_steps=55 model_name=.robot_arm"
!
...
constraint modify motion_generator  &
   motion_name = .robot_arm.motor_1  &
   function = "step(time,0,0d,1,-40d)+step(time,2,0d,3,20d)+step(time,3,0d,4,-20d)+step(time,4.1,0d,5,20d)+step(time,6,0d,7,-35d)"
!
constraint modify motion_generator  &
   motion_name = .robot_arm.motor_4  &
   function = "step(time, 2.5, 0.0d, 3.5, 90.0d)+STEP(time, 5.1, 0.0d, 6.0, -90.0d)"
!
constraint modify motion_generator  &
   motion_name = .robot_arm.motor_2  &
   function = "step(time,0,0d,1,-110d)+step(time,2,0d,3,40d)+step(time,3,0d,4,-40d)+step(time,4.1,0d,5,40d)+step(time,6,0d,7,-5d)"
!
constraint modify motion_generator  &
   motion_name = .robot_arm.motor_3  &
   function = "step(time,0,0d,1,60d)+step(time,2,0d,3,-60d)+step(time,3,0d,4,60d)+step(time,4.1,0d,5,-60d)+step(time,6,0d,7,40d)"
!
force modify direct single_component_force  &
   single_component_force_name = .robot_arm.gripper_torque  &
   function = "step(time,1,0,1.1,-8000)+STEP(time, 4.0, 0.0, 4.1, 8000.0)"
!
!-------------------------- Adams View UDE Instance ---------------------------!
!
!
